...
float kp = 40, ki = 100, kd = 0;
PositionPID pidLeft(0, cmdMaxVal, kp, ki, kd);
PositionPID pidRghit(0, cmdMaxVal, kp, ki, kd);
...
//    }
    ...
    if (fabs(cmd_motor_left) == 0 && fabs(cmd_motor_right) == 0) stope();
     ...
     if (dt >= pidDuration) {
       lastPidTime = millis();
       pidRghit.setSetPoint(cmd_motor_right);
       pidLeft.setSetPoint(cmd_motor_left);
       motor_right = pidRghit.calc(angularVelRKF.vel_kf, dt);
       if (!pidLeft.dir) motor_right = -motor_right;
       motor_left = pidLeft.calc(angularVelLKF.vel_kf, dt);
       if (!pidLeft.dir) motor_left = -motor_left;
       if (debug==1){
         Serial2.println("Debug;cmd: cmdRight - " + (String)(motor_right) + " , " + (String)(angularVelRKF.vel_kf) + " , cmdLeft - " + (String)(motor_left) + " , " + (String)(angularVelLKF.vel_kf));
       }
     }
